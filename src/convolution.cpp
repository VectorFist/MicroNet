...
void __forward__(Chunk* input, Chunk* output, Chunk* weights, Chunk* bias, int start_n, int end_n,
...
void Convolution::forward(const vector<Chunk*>& input, const vector<Chunk*>& output) {
    ...
    if (num < 4) {
        __forward__(input[0], output[0], params_[0].get(), params_[1].get(), 0, num,
        ...
        for (int i = 0; i < 4; ++i) {
            threads.push_back(thread(__forward__, input[0], output[0], params_[0].get(), params_[1].get(),
                                     i*block, min(num, (i+1)*block),
                                     output_channels, output_h, output_w, input_channels,
                                     input_h, input_w, kernel_h_, kernel_w_, pad_h_, pad_w_,
                                     stride_h_, stride_w_));
        }
...
void __backward__(Chunk* input, Chunk* output, Chunk* weights, Chunk* bias, int start_n, int end_n,
                  int output_channels, int output_h, int output_w, int input_channels,
                  int input_h, int input_w, int kernel_h, int kernel_w, int pad_h, int pad_w,
                  int stride_h, int stride_w) {
    const float* input_data = input->const_data();
    const float* weights_data = weights->const_data();
    const float* output_diff = output->const_diff();
    float* input_diff = input->diff();
    float* weights_diff = weights->diff();
    float* bias_diff = bias->diff();
...
void Convolution::backward(const vector<Chunk*>& input, const vector<Chunk*>& output) {
    ...
    if (num < 4) {
        __backward__(input[0], output[0], params_[0].get(), params_[1].get(), 0, num,
                     output_channels, output_h, output_w, input_channels, input_h, input_w,
                     kernel_h_, kernel_w_, pad_h_, pad_w_, stride_h_, stride_w_);
    } else {
        vector<thread> threads;
        vector<Chunk*> inputs_vec, weights_vec, bias_vec;
        for (int i = 0; i < 4; ++i) {
            inputs_vec.push_back(new Chunk(*input[0]));
            weights_vec.push_back(new Chunk(*params_[0]));
            bias_vec.push_back(new Chunk(*params_[1]));
        }

        int block = int(ceil(num / 4.0));
        for (int i = 0; i < 4; ++i) {
            threads.push_back(thread(__backward__, inputs_vec[i], output[0], weights_vec[i], bias_vec[i],
                                     i*block, min(num, (i+1)*block),
                                     output_channels, output_h, output_w, input_channels,
                                     input_h, input_w, kernel_h_, kernel_w_, pad_h_, pad_w_,
                                     stride_h_, stride_w_));
        ...
        vector<const float*> inputs_diff_vec = {inputs_vec[0]->const_diff(), inputs_vec[1]->const_diff(),
                                                inputs_vec[2]->const_diff(), inputs_vec[3]->const_diff()};
        vector<const float*> weights_diff_vec = {weights_vec[0]->const_diff(), weights_vec[1]->const_diff(),
                                                 weights_vec[2]->const_diff(), weights_vec[3]->const_diff()};
        vector<const float*> bias_diff_vec = {bias_vec[0]->const_diff(), bias_vec[1]->const_diff(),
                                              bias_vec[2]->const_diff(), bias_vec[3]->const_diff()};
        for (int i = 0; i < input[0]->count(); ++i) {
            for (int j = 0; j < 4; ++j) {
                input_diff[i] += inputs_diff_vec[j][i];
            }
        }
        for (int i = 0; i < params_[0]->count(); ++i) {
            for (int j = 0; j < 4; ++j) {
                weights_diff[i] += weights_diff_vec[j][i];
            }
        }
        for (int i = 0; i < params_[1]->count(); ++i) {
            for (int j = 0; j < 4; ++j) {
                bias_diff[i] += bias_diff_vec[j][i];
            }
        }

        for (int i = 0; i < 4; ++i) {
            delete inputs_vec[i];
            delete weights_vec[i];
            delete bias_vec[i];
        }
    }
